...
function uuid() {
	return Array.from({ length: 36 }).map((_, i) => {
		...
		else return Math.floor(Math.random() * 16).toString(16);
	}).join("");
}
...
var LRUCache = class {
	...
	clear() {
		for (const key of Object.keys(this.entries)) this.evict(key);
	}
	read(key) {
	...
	trim() {
		for (const key of this.keys.splice(this.size)) this.evict(key);
	}
	evict(key) {
...
var FormSubmission = class FormSubmission {
	...
	async start() {
		...
		if (typeof confirmationMessage === "string") {
			if (!await (typeof config.forms.confirm === "function" ? config.forms.confirm : FormSubmission.confirmMethod)(confirmationMessage, this.formElement, this.submitter)) return;
		}
...
function getCookieValue(cookieName) {
	if (cookieName != null) {
		const cookie = (document.cookie ? document.cookie.split("; ") : []).find((cookie) => cookie.startsWith(cookieName));
		if (cookie) {
...
var FormLinkClickObserver = class {
	...
	followedLinkToLocation(link, location) {
		...
		const type = "hidden";
		for (const [name, value] of location.searchParams) form.append(Object.assign(document.createElement("input"), {
			type,
			name,
			value
		}));
	...
	const morphNode = (function() {
		...
		*/
		function morphNode(oldNode, newContent, ctx) {
			...
			else if (oldNode instanceof HTMLHeadElement && ctx.head.style !== "morph") handleHeadElement(oldNode, newContent, ctx);
			else {
...
var DefaultIdiomorphCallbacks = class {
	...
	beforeNodeMorphed = (currentElement, newElement) => {
		if (currentElement instanceof Element) {
			if (!currentElement.hasAttribute("data-turbo-permanent") && this.#beforeNodeMorphed(currentElement, newElement)) return !dispatch("turbo:before-morph-element", {
				cancelable: true,
...
var MorphingFrameRenderer = class extends FrameRenderer {
	static renderElement(currentElement, newElement) {
		...
		morphChildren(currentElement, newElement, { callbacks: { beforeNodeMorphed: (node, newNode) => {
...
var PageSnapshot = class PageSnapshot extends Snapshot {
	...
	clone() {
		...
		for (const clonedNoscriptElement of clonedElement.querySelectorAll("noscript")) clonedNoscriptElement.remove();
...
var Visit = class {
	...
	constructor(delegate, location, restorationIdentifier, options = {}) {
		...
		const { action, historyChanged, referrer, snapshot, snapshotHTML, response, visitCachedSnapshot, willRender, updateHistory, shouldCacheSnapshot, acceptsStreamResponse, direction, refresh } = {
			...defaultOptions,
			...options
		};
		this.action = action;
	...
	loadCachedSnapshot() {
		...
		if (snapshot) {
			...
			this.render(async () => {
				this.cacheSnapshot();
				if (this.isPageRefresh) this.adapter.visitRendered(this);
				else {
					if (this.view.renderPromise) await this.view.renderPromise;
	...
	shouldIssueRequest() {
		if (this.action == "restore") return !this.hasCachedSnapshot();
...
var BrowserAdapter = class {
	...
	visitProposedToLocation(location, options) {
		if (locationIsVisitable(location, this.navigator.rootLocation)) this.navigator.startVisit(location, options?.restorationIdentifier || uuid(), options);
	...
	visitRequestCompleted(visit) {
		visit.loadResponse();
		if (visit.response.redirected) this.redirectedToLocation = visit.redirectedToLocation;
	}
...
var Navigator = class {
	...
	proposeVisit(location, options = {}) {
		if (this.delegate.allowsVisitingLocationWithAction(location, options.action)) this.delegate.visitProposedToLocation(location, options);
	}
	...
	async formSubmissionFailedWithResponse(formSubmission, fetchResponse) {
		...
		if (responseHTML) {
			...
			if (snapshot.refreshScroll !== "preserve") this.view.scrollToTop();
	...
	linkPrefetchingIsEnabledForLocation(location) {
		if (typeof this.adapter.linkPrefetchingIsEnabledForLocation === "function") return this.adapter.linkPrefetchingIsEnabledForLocation(location);
		return true;
	}
	visitStarted(visit) {
...
var PageRenderer = class extends Renderer {
	...
	#setLanguage() {
		...
		if (dir) documentElement.setAttribute("dir", dir);
		else documentElement.removeAttribute("dir");
	}
	async mergeHead() {
	...
	removeNoscriptElements() {
		for (const noscriptElement of this.newElement.querySelectorAll("noscript")) noscriptElement.remove();
	}
...
var MorphingPageRenderer = class extends PageRenderer {
	static renderElement(currentElement, newElement) {
		morphElements(currentElement, newElement, { callbacks: { beforeNodeMorphed: (node, newNode) => {
...
var PageView = class extends View {
	...
	renderPage(snapshot, isPreview = false, willRender = true, visit) {
		const renderer = new (this.isPageRefresh(visit) && (visit?.refresh?.method || this.snapshot.refreshMethod) === "morph" ? MorphingPageRenderer : PageRenderer)(this.snapshot, snapshot, isPreview, willRender);
...
var Preloader = class {
	...
	async preloadURL(link) {
		...
		if (this.snapshotCache.has(location)) return;
		await new FetchRequest(this, FetchMethod.get, location, new URLSearchParams(), link).perform();
	}
...
var Session = class {
	...
	applicationAllowsFollowingLinkToLocation(link, location, ev) {
		return !this.notifyApplicationAfterClickingLinkToLocation(link, location, ev).defaultPrevented;
	}
	applicationAllowsVisitingLocation(location) {
		return !this.notifyApplicationBeforeVisitingLocation(location).defaultPrevented;
	}
	...
	submissionIsNavigatable(form, submitter) {
		if (config.forms.mode == "off") return false;
		else {
			const submitterIsNavigatable = submitter ? this.elementIsNavigatable(submitter) : true;
			if (config.forms.mode == "optin") return submitterIsNavigatable && form.closest("[data-turbo=\"true\"]") != null;
...
var FrameController = class {
	...
	disabledChanged() {
		if (this.disabled) this.#currentFetchRequest?.cancel();
		else if (this.loadingStyle == FrameLoadingStyle.eager) this.#loadSourceURL();
	}
	sourceURLChanged() {
		if (this.#isIgnoringChangesTo("src")) return;
		if (!this.sourceURL) this.#currentFetchRequest?.cancel();
	...
	async loadResponse(fetchResponse) {
		...
		try {
			...
			if (html) {
				...
				if (PageSnapshot.fromDocument(document).isVisitable) await this.#loadFrameResponse(fetchResponse, document);
	...
	async #visitResponse(response) {
		...
		return session.visit(location, { response: {
			redirected,
			statusCode,
			responseHTML
		} });
	...
	#shouldInterceptNavigation(element, submitter) {
		...
		if (id) {
			...
			else if (id == "_parent") return false;
		}
